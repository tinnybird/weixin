...
class ProductAction extends BaseAction{
	...
	public function orderCart(){
		...
		if (IS_POST){
			...
			//新增订单 给商家发送信息
			$content = -1;
			$info=M('Wxuser_setting')->where(array('token'=>$this->token))->find();
			// 增加 给商家发送短信
			$phone_info = json_decode($info['sms'], true);
			
			if ($phone_info['t'] && $this->isMobile($phone_info['t']) && $phone_info['s'] == 1) {
				$content = $this->sms();
			    if ($content) {
			        $smsrs = file_get_contents('http://api.smsbao.com/sms?u='.$phone_info['u'].'&p='.$phone_info['p'].'&m='.$phone_info['t'].'&c='.urlencode($content));
			    }
			}
			// 结束

			// 增加 给商家发送邮件
			$mail_info = json_decode($info['email'], true);
			if ($mail_info['s'] == 1 && $mail_info['uc'] && $mail_info['ur']) {
			    if($content == -1) $content = $this->sms();
			    if ($content) {
			        ...
			        $mail->Host = $mail_info['ht'];
			        $mail->Port = $mail_info['pt'];
			        $mail->Username = $mail_info['uc'];
			        $mail->Password = $mail_info['pw'];
			        $mail->SetFrom($mail_info['uc'], $mail_info['un']);
			        $mail->AddReplyTo($mail_info['ur'], $mail_info['un']);
			        $mail->Subject = '客户订单通知';
			        $mail->AltBody = '';
			        $mail->MsgHTML($content);
			        $mail->AddAddress($mail_info['ur'], '微信平台');
			        $re = $mail->Send();
				}
			}
			// 结束
